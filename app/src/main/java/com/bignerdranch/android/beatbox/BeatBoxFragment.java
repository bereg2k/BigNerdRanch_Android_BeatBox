...
public class BeatBoxFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        binding.playbackSpeedTextView.setText(
                getString(R.string.playback_speed,
                        binding.seekBarPlaybackSpeed.getProgress())
        );

        binding.seekBarPlaybackSpeed.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                mBeatBox.setPlaybackRate(progress / 100F);
                mBeatBox.changePlaybackRateOnTheFly();
                binding.playbackSpeedTextView.setText(getString(R.string.playback_speed, progress));
            }
